...
//	} RxByteBuffer; 
	...
	BL_TAU0_Channel0_Start();
	rxstatus = PurgeComms( 2 );
				...
				else
				{
					...
					else
					{
						...
						if ( RxByteBuffer.uc[1 + 1] == ExpectedBlkNum )
						{
							...
							bl_fsl_write_str.fsl_data_buffer_p_u08 = (fsl_u08 *)&RxByteBuffer.uc[3 + 1];
							bl_fsl_write_str.fsl_word_count_u08 = 0x20;
							bl_fsl_write_str.fsl_destination_address_u32 = Address;
							bl_fsl_status = FSL_Write((__near fsl_write_t*)&bl_fsl_write_str);
							if(bl_fsl_status != FSL_OK) 
							MyErrorHandler();
							
							//bl_fsl_status = FSL_IVerify(iverify_blk);
							//iverify_blk++;
							if(bl_fsl_status == FSL_OK) {
